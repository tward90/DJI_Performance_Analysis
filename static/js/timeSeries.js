function init() {
    ...
    d3.json("/time_series").then( timeData => {
    ...
    //     d3.json("googl.json").then( googlData => {
            ...
            const dateMin = d3.min(
                d3.min(dataArr.map(d => d.map(j => new Date(j.date_close))))
            );
            const dateMax = d3.max(
                d3.max(dataArr.map(d => d.map(j => new Date(j.date_close))))
            );
            ...
            dataArr.map(d => {
                console.log(dateMin);
                console.log(d.find(element => element.date_close === dateMin));

                let localMinDate = d3.min(d.map(k => new Date(k.date_close)));
                console.log(localMinDate);
                let c0 = d.find(element => new Date(element.date_close) === localMinDate).close;
                d.map(j => {
